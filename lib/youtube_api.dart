...
class YoutubeAPI {
  ...
  Future<List<YT_API>> channel(String channelId, {String order,String query}) async {
    this.getTrending = false;
    Uri url = api.channelUri(channelId, order,query: query);
    var res = await http.get(url, headers: {"Accept": "application/json"});
    var jsonData = json.decode(res.body);
    print(jsonData["nextPageToken"]);
    if (jsonData['error'] != null) {
      throw jsonData['error']['message'];
    }
    if (jsonData['pageInfo']['totalResults'] == null) return <YT_API>[];
  ...
  Future<List<YT_API>> channelPlayLists(String channelId, {String order}) async {
    this.getTrending = false;
  ...
  Future<List<YT_API>> playList(String listId, {String order}) async {
    this.getTrending = false;
    Uri url = api.listUri(listId, order);
    var res = await http.get(url, headers: {"Accept": "application/json"});
    var jsonData = json.decode(res.body);
    if (jsonData['error'] != null) {
      throw jsonData['error']['message'];
  ...
  Future<List<YT_API>> nextPage() async {
    ...
    List<YT_API> result = [];
    print(nextPageToken);
    Uri url = api.nextPageUri(this.getTrending , nextPageToken);
    var res = await http.get(url, headers: {"Accept": "application/json"});
    var jsonData = json.decode(res.body);
    ...
    api.setNextPageToken(jsonData['nextPageToken']);
    api.setPrevPageToken(jsonData['prevPageToken']);
    print(jsonData['pageInfo']['totalResults']);
    print(jsonData['pageInfo']['resultsPerPage']);
    print(jsonData['nextPageToken']);
    int total = jsonData['pageInfo']['totalResults'] <
            jsonData['pageInfo']['resultsPerPage']
        ? jsonData['pageInfo']['totalResults']
        : jsonData['pageInfo']['resultsPerPage'];
    result = await _getListOfYTAPIs(jsonData, jsonData['items'].length);
    page++;
    if (total == 0) {
      return <YT_API>[];
    }
